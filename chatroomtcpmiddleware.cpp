...
https://firstdonoharm.dev/version/3/0/cl-mil.txt
...
ChatRoomTcpMiddleware::ChatRoomTcpMiddleware(QObject *parent) : QObject(parent),
    m_server(new QTcpServer(parent))
{
    // Created QTcpSocketServer is connectet to the
    // ChatRoom's processNewConnections slot
    QObject::connect(m_server, &QTcpServer::newConnection,
                     this, &ChatRoomTcpMiddleware::processNewConnection);
}
...
void ChatRoomTcpMiddleware::processNewConnection()
{
    // Accept incoming connection
    QTcpSocket* socket = m_server->nextPendingConnection();
    ChatRoomUser* user = new ChatRoomUser(parent(), socket->peerAddress());
    SocketWrapper* wrapper = new SocketWrapper(socket, user);
...
void ChatRoomTcpMiddleware::listen(const uint port,
